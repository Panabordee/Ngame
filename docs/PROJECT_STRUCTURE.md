...
| Path | Responsibility |
| --- | --- |
| `client/` | React/Vite card-table client, auth controls, room actions, and original art |
...
| `docker-compose.yml` | Frontend, API, realtime, PostgreSQL, and Redis orchestration |
...
- `server/src/guestSessions.ts`: one-match Guest reservation/commit registry with Redis and local in-memory implementations.
