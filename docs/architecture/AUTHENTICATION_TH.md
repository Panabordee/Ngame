...
5. Realtime จอง Guest session ให้ห้องแรก ถ้าออกก่อน host เริ่มจะคืน reservation แต่เมื่อเริ่มเกมแล้ว session จะถูก commit และ JWT นั้นเข้าห้องเกมที่สองไม่ได้
...
Guest ไม่สร้างข้อมูลในตารางเหล่านี้ identity และ expiry อยู่ใน JWT ที่ลงนาม ส่วนการผูกหนึ่งเกมเป็น state ของ realtime process
...
- Guest registry แบบ in-memory ตรงกับ deployment ปัจจุบันที่มี realtime instance เดียว ถ้าจะรันหลาย replica ต้องมี distributed Colyseus matchmaker และ shared atomic Guest registry
