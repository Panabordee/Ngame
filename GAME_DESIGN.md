...
- Every player's final selected card becomes a revealed card in their initial rack and counts toward their hand size. A standard selected card is inserted automatically in legal order. A selected Joker remains revealed and its owner must choose any rack position before turn one.
...
- The host may set the turn timer to Off, 30, 60, 90, 120, 180, or 300 seconds. Clients display the server deadline as a countdown. On timeout: a player who has not drawn loses the turn; a pending card is revealed and inserted automatically into a legal slot; and an empty-pile guess or self-penalty timeout reveals a server-randomized unrevealed card from that player's rack. The timer pauses during reconnection.
